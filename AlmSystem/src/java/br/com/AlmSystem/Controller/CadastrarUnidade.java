...
public class CadastrarUnidade extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            unidade.setDescUnidade(descri);

            try {
                GenericDAO dao = new UnidadeDAOImpl();
                if (dao.cadastrar(unidade)) {
                    mensagem = "Tipo da unidade cadastrado com sucesso";
                } else {
                    mensagem = "Problemas ao cadastrar a unidade Servelt";
                }
                request.setAttribute("mensagem", mensagem);
                request.getRequestDispatcher("unidade/cadastrarunidade.jsp").forward(request, response);
            } catch (Exception e) {
