...
public class CadastrarUsuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            usuario.setTipoPessoa(tipo);
            usuario.setEmailPessoa(email);
            usuario.setTelefonePessoa(telefone);
            usuario.setCepPessoa(cep);
            usuario.setCidadePessoa(cidade);
            usuario.setLogadoroPessoa(logradouro);
            usuario.setBairroPessoa(bairro);
            ...
            try {
                ...
                if (dao.cadastrar(usuario)) {
                    ...
                    mensagem = "Problemas ao cadastrar usuario servelt";
                }
                request.setAttribute("mensagem", mensagem);
                request.getRequestDispatcher("index.jsp").forward(request, response);
            } catch (Exception e) {
                System.out.println("Problemas no Servlet ao cadastrar Pessoa Servelt! Erro: " + e.getMessage());
                e.printStackTrace();
            }
        }
    }
